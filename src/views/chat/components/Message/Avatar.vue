<template>
    <!-- 根据image确定展示用户头像还是chatGPT icon -->
    <template v-if="image">
        <el-avatar v-if="isString(avatar) && avatar.length > 0" size="small" :src="avatar" @error="errorHandler">
            <!-- 加载失败时显示默认的图片 -->
            <img :src="defaultAvatar">
        </el-avatar>
        <el-avatar v-else :size="30" :src="defaultAvatar" />
    </template>
    <span v-else class="text-[28px] text-gray-500">
